...
(function (api) {
	...
	api.sectionConstructor['spice-software-customizer-notify-section'] = api.Section.extend(
		{
				...
				return true;
					...
					jQuery( document ).ready(
						function () {
							...
							jQuery( '.spice-software-customizer-notify-dismiss-recommended-action' ).click(
								function () {
									...
									jQuery.ajax(
										{
											type: 'GET',
											data: {action: 'spice_software_customizer_notify_dismiss_action', id: id, todo: action},
											dataType: 'html',
											...
											success: function (data) {
												...
												var actions_count      = recommended_sction.find( '.spice-software-customizer-plugin-notify-actions-count' );
												var section_title      = recommended_sction.find( '.section-title' );
												jQuery( '.spice-software-customizer-plugin-notify-actions-count .current-index' ).text( index );
												container.slideToggle().remove();
												if (jQuery( '.spice-software-theme-recomended-actions_container > .epsilon-recommended-actions' ).length === 0) {

													actions_count.remove();

													if (jQuery( '.spice-software-theme-recomended-actions_container > .epsilon-recommended-plugins' ).length === 0) {
										...
										jQuery( '.spice-software-customizer-notify-dismiss-button-recommended-plugin' ).click(
											function () {
												var id = jQuery( this ).attr( 'id' ),
												action = jQuery( this ).attr( 'data-action' );
												jQuery.ajax(
...
jQuery( document ).ready(
	function ($) {
		$( 'body' ).on(
			'click', ' .spice-software-install-plugin ', function () {
