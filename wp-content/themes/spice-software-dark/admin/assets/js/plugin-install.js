jQuery(document).ready(function($) {
    'use strict';
    var this_obj = spice_software_dark_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'spice_software_dark_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.spicethemes-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'spice_software_dark_hide_customizer_notice';
            sdata.chilly_plugin_slug = slug;
        }else{
            sdata.action = 'spice_software_dark_hide_customizer_companion_notice';
        }
...
jQuery(function(){
	var $mainSpice_Software_Dark=jQuery('#main-content');
	...
	$cat_links.on('click',function(e){
		...
		$mainSpice_Software_Dark.animate({opacity:"0.5"});
		$mainSpice_Software_Dark.load(value + "#getting_started",function(){
		$mainSpice_Software_Dark.animate({opacity:"1"});	
		});
	});
});