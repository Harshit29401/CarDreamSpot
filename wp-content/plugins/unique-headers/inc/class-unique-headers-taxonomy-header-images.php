<?php
/**
 * Unique_Headers_Taxonomy_Header_Images Class File
 *
 * This class provides methods for adding and managing custom header images
 * for different taxonomies.
 *
 * @package    UniqueHeaders
 * @subpackage TaxonomyHeaderImages
...
class Unique_Headers_Taxonomy_Header_Images extends Unique_Headers_Core {
	...
	public function storing_taxonomy_data() {
		...
		$tag_ID        = absint( wp_unslash( $_POST['tag_ID'] ) );
		$attachment_id = absint( wp_unslash( $_POST[ $this->name . '-id' ] ) );
		if ( is_numeric( $attachment_id ) ) {
	...
	private function get_attachment_id_from_url( $url = '' ) {
		...
		if ( false !== strpos( $url, $upload_dir_paths['baseurl'] ) ) {
			...
			$cache_key     = 'unique_header_query_' . md5( $url );
	...
	public function extra_fields() {
		...
		$tag_ID        = absint( filter_input( INPUT_GET, 'tag_ID' ) );
